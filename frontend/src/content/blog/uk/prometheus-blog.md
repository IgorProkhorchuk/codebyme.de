...
Prometheus не врятує тебе від аварій, але зробить так, що ти дізнаєшся про проблему *раніше за клієнта*.
...
Кожен блок у `scrape_configs` — це окремий *job*, тобто логічна група таргетів. Наприклад, один job для самого Prometheus (так, він також моніторить сам себе), інший — для системних метрик сервера.
...
Ця ідея — маленький окремий процес, який «знає» про одну конкретну систему і вміє відповідати Prometheus — називається *exporter*. Для баз даних, черг повідомлень, nginx, redis — для всього є свої exporters. Це дуже розширювана архітектура.
